...
public class IntElement extends GuiElement {
    ...
    public IntElement(UrbexConfigScreen gui, String page, int x, int y, String attribute) {
        super(page, x, y);
        this.gui = gui;
        this.attribute = attribute;
        Integer c = gui.getLocalSetup().get().map(h -> (Integer) h.toConfiguration().get(attribute)).orElse(0);
        field = new EditBox(gui.getFont(), x, y, 45, 16, Component.literal(Integer.toString(c))) {
            // @todo 1.19.3
//            @Override
