...
- **Lighting density now reaches the lights a pack actually places.** A light used to be a separate
  *kind* of palette entry — one carrying `torch: true` or a typed `light` pool — so any emitting
  ...
  how most packs author their lighting. ModernTweaks writes all of its as `E`/`e` lantern entries,
  826 marker positions across 197 files, and its parts contain no `T` marker at all: moving the
  slider from 0 to 1 changed nothing visible in a ModernTweaks world.

  Any palette entry can now carry `lightSource`, and doing so puts it under `lightingDensity`.
  Nothing is filtered out of the output any more: the marker writes either its light or the
  **replacement** the entry names.

  - `"lightSource": true` on an ordinary entry makes that block optional in place, written exactly
    where and as the author wrote it. `unlit` (or the weighted `unlitBlocks`) is what stands there
    when it is off; name neither and it is air, which is what a rejected marker always left behind.
  - A `lightSource` declaring `floor`, `wall`, `ceiling` or `free` candidates is a *socket* — the
    old `light` pool, renamed, still support-searched and oriented by the deferred placer.
  - A socket candidate names its **own** `unlit`, because an unlit torch on a wall and an unlit
    torch on a floor are two different blocks. Both passes draw from one stream at one position, so
    raising the density lights the fixture already standing there rather than moving it. In the
    bundled pack a torch that is off is a spent candle and a hanging lantern is the chain it hung
    from.
  - **Breaking:** `torch` and `light` are removed. Both still decode, and a palette using either
  ...
  - The `urbex:lights` block tag is gone. Its one job was deciding whether placing a state has to
    ...
    mob or NBT never scheduled its update at all.

  Nothing is a light unless its entry says so: redstone torches wired into doors, brewing stands,
  glow lichen and campfires authored `lit=false` stay ordinary blocks that the density cannot touch.
  On the oil rig, `J` sea lanterns leave prismarine bricks and `|` redstone wall torches go dark
  rather than vanishing. All five digest goldens moved; they were regenerated by deleting them and
  running twice for agreement, and ten suites pass at the default worker count with
  `runDigestCheck`/`Avoid`/`Rail` agreeing again at `-Dmax.bg.threads=2`.
